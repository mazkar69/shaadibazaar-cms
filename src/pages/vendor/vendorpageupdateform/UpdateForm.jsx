...
export default function UpdateForm() {
    ...
    useEffect(()=>{
        const getVendorPageDetails = async()=>{
            try {
                const { data } = await authApi.get(`/api/vendor/page/list/?_id=${_id}`)
                if(data.success){
                    const page = data.data.vendorPages[0];
                    setFormData(page)
    ...
    const handleSubmit = async()=>{
        ...
        try {
                ...
                const { data } = await authApi.post(`/api/vendor/page/update/${_id}`,{...formData,url})
